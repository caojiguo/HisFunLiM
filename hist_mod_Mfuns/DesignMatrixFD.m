...
if DomainType == 1
    ...
    mm3   = 0;
    mm4   = 1;
    m     = 1;  % index for current "row", from 1 to M
    for it = 1:ntpts
        t   = tpts(it);  %  current value of t
        if m < ceil(it/npts)
            if m <= B-lag
        ...
        if m <= B-lag
            ...
            tdelta    = t - rghttver;
            sval      = [0;      tdelta];  % location in this element
            tval      = [tdelta; tdelta];  % location in this element
            ...
            basisR    = create_power_basis(rangeR, 2);  
                        % 2 is number of basis, which leads to (1, x)
            ...
            for b = 1:(m-1)   % (m-1) pairs of elements
                %  left element
                ielem = ielem + 1;
