%% scenario 3:
%  beta(s,t) is linear over the trapezoidal region, with holes [simu2+holes]
...
for j=1:(M+1)
    ...
    if j<=(M-deltaM+1)
        ...
        tmp(logical(eye(j))) = (j-(M-deltaM+1))*S/deltaM * binornd(1, p1, j, 1);
    end
    bmat(1:j, 1:j) = tmp(1:j, j:-1:1);
end

% stretch from matrix to vector form
bvec_true = reshape_bmat2vec(bmat, M);
...
%% Tuning parameters
% discrete penalty
wH_lma = exp(linspace(-15, -14, 5));

% seq of penalty parameters for GBR approach
tau = exp(linspace(-22, -21, 10));
wH_gbr = exp(linspace(-15, -14, 5)); 
